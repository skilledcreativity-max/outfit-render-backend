...
const GLYPH_MAP = Object.freeze({
  ...
  "3": [0x3C, 0x02, 0x02, 0x1C, 0x02, 0x02, 0x3C],
  "4": [0x24, 0x24, 0x24, 0x3E, 0x04, 0x04, 0x04],
  "5": [0x3E, 0x20, 0x20, 0x3C, 0x02, 0x02, 0x3C],
  ...
  "@": [0x1C, 0x22, 0x2E, 0x2A, 0x2E, 0x20, 0x1E],
  "%": [0x22, 0x24, 0x08, 0x10, 0x20, 0x12, 0x22],
...
async function drawTextStroke(layer, stroke) {
  const cleanText = String(stroke.text || "").toUpperCase().slice(0, MAX_TEXT_LENGTH);
  if (!cleanText) return;
  const numChars = cleanText.length;
  const totalUnitsX = numChars * 6 - 1;
  const totalUnitsY = 7;
  const scaleX = stroke.width / totalUnitsX;
  const scaleY = stroke.height / totalUnitsY;
  const pixelScale = Math.max(1, Math.min(scaleX, scaleY));

  const textWidth = Math.ceil(totalUnitsX * pixelScale);
  const textHeight = Math.ceil(totalUnitsY * pixelScale);
  const textCanvas = new Jimp({ width: Math.max(1, stroke.width), height: Math.max(1, stroke.height), color: 0x00000000 });

  const startX = Math.max(0, Math.floor((stroke.width - textWidth) / 2));
  const startY = Math.max(0, Math.floor((stroke.height - textHeight) / 2));
  const isScript = stroke.font === "Script";
  const textColor = cssColorToHex(stroke.colorHex || "#FFFFFF");

  for (let charIdx = 0; charIdx < numChars; charIdx += 1) {
    const ch = cleanText[charIdx];
    const glyph = GLYPH_MAP[ch] || GLYPH_MAP["?"];
    const charStartX = startX + Math.floor(charIdx * 6 * pixelScale);
    if (glyph) {
      for (let rowIdx = 0; rowIdx < 7; rowIdx += 1) {
        const rowBits = glyph[rowIdx];
        const slant = isScript ? Math.floor((6 - rowIdx) * (pixelScale * 0.28)) : 0;
        for (let colIdx = 0; colIdx < 5; colIdx += 1) {
          const bitMask = 1 << (5 - colIdx);
          if ((rowBits & bitMask) !== 0) {
            const px = charStartX + Math.floor(colIdx * pixelScale) + slant;
            const py = startY + Math.floor(rowIdx * pixelScale);
            const w = Math.ceil(pixelScale);
            const h = Math.ceil(pixelScale);
            for (let dy = 0; dy < h; dy += 1) {
              for (let dx = 0; dx < w; dx += 1) {
                const targetX = px + dx;
                const targetY = py + dy;
                if (targetX >= 0 && targetX < stroke.width && targetY >= 0 && targetY < stroke.height) {
                  textCanvas.setPixelColor(textColor, targetX, targetY);
                }
              }
            }
          }
  ...
  if (stroke.rotation && stroke.rotation !== 0) {
...
async function renderDesign(design) {
  ...
  return await baseLayer.getBuffer(JimpMime.png || "image/png");
}
